...
public class ThiefRobot extends Robot
{
	...
	private final int newheight = _size + 5;

	public ThiefRobot (final int globalId, final int localId, final boolean display)
	{
		super(globalId, localId, display);
		this.body = new Rectangle(_colors[localId], _coordinates[localId][0], _coordinates[localId][1], _size, newheight);
	...
	public void move (final MoveRobotContext cntx)
	{
		...
		if (cntx.victims == null) { return; }

		for (final Robot vic: cntx.victims)
		{		
			final int amount = vic.getProfit();
			this.increaseProfit(amount);
			vic.increaseProfit(-1 * amount);
		}
	}
}
